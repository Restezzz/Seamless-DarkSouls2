...
//     soon as the guest stands at home alive -- the sign aimed at the respawn
//     spot: where the guest last rested at a bonfire in the host's world, else
//     beside the bonfire nearest to where the guest first appeared there, else
//     the host's feet. When it was the HOST who died, the spot is where the
//     guest was standing.
...
// EventBossBattleManager [[GMImp+0x70]+0x88]: +0x14 battles running (what the
// game's own IsBossBattle check reads), +0x204 phase (1 fighting, 2 defeated,
// 3 cleanup).
...
bool ReadLocalSpot(Spot* Out) {
    ...
    *Out = Spot{ Hooks::GetLocalAreaId(), X, Y, Z, true };
    return Out->Area != 0;
}
...
void __fastcall PhantomBranchDetour(void* Result, int Reason) {
    ...
    const bool InBoss   = BossActive > 0 && BossPhase == 1;
    ...
    LOG_INFO("[DEATH] phantom branch: reason %d, my HP %d, join state %d, boss %d (phase %d), partner %s -> %s",
             Reason, Hp, Join, BossActive, BossPhase, Partner ? "alive" : "dead",
...
void TickLife(int32_t Hp) {
    ...
    const Session::SessionPlayer* Local = Players.GetLocalPlayer();
...
void TickBoss() {
    ...
    if (Active == g_lastBossActive && Phase == g_lastBossPhase) return;
...
void TickArrival(int Join) {
    ...
    LOG_INFO("[DEATH] arrived in the partner's world at (%.2f, %.2f, %.2f), map %u; nearest bonfire spot (%.2f, %.2f, %.2f), %.0f m away",
             Here.X, Here.Y, Here.Z, Here.Area, B[0], B[1], B[2], Len);
}
...
void TickHold(int Join, int32_t Hp) {
    ...
    if (!InBossFight())                                  Why = "the boss fight is over";
...
void TickRejoin(int Join, int32_t Hp) {
    ...
    if (Now - g_rejoin.Since > kRejoinGiveUpMs) {
        g_rejoin.Pending = false;
        LOG_WARNING("[DEATH] never got home alive in 3 minutes -- not joining again by myself");
        return;
    }
